...
void HelloWorld::initPlayer() {
	playerMain = new Player(MAIN, this, ccp(15,15+20), 22);//字母数字20*20
	playerSubordinate = new Player(SUBORDINATE, this, ccp(277,67+16), 18);//字母数字16*16
}
...
void HelloWorld::ccTouchesEnded(CCSet* touches, CCEvent* event) {
	...
	if(startLocation.x > playerMain->boardPos.x
		...
		if(plane != NULL) {
			...
			} else if(startLocation.x-location.x > playerMain->lenPerTile && fabs(startLocation.y-location.y) < playerMain->lenPerTile) {	//左移
				playerMain->move(plane, -1, 0);
			} else if(location.x-startLocation.x > playerMain->lenPerTile && fabs(startLocation.y-location.y) < playerMain->lenPerTile) {	//右移
				CCLog("***********right");
				CCLog("***********plane:%f", plane->pos.x);
				playerMain->move(plane, 1, 0);
			} else if(fabs(location.x-startLocation.x) < playerMain->lenPerTile && location.y-startLocation.y > playerMain->lenPerTile) {	//上移
				playerMain->move(plane, 0, 1);
			} else if(fabs(location.x-startLocation.x) < playerMain->lenPerTile && startLocation.y-location.y > playerMain->lenPerTile){	//下移
				playerMain->move(plane, 0, -1);
			}
		}
	}
